...
{\color{udsect}
We present the first implementation \cite{fp} of the hadronic cascade model {\sf INCL 4.2}
\cite{incl} and evaporation model {\sf ABLA V3} \cite{abla} in {\sf Geant4 9.1} \cite{g4}.
\vskip0.5cm

{\sf INCL} and {\sf ABLA} models play an important role in nuclear physics
applications, especially in spallation reaction simulations.  The
inclusion of these models into {\sf Geant4} makes it possible to use them
for LHC detector simulations. Additional uses for the models are:
\begin{itemize}
...
{\sf INCL} introduces \emph{only two free parameters}:
\begin{itemize}
...
{\Large {\sf Double-differential for neutron production cross section
    from {\sf Geant4} {\sf INCL} and {\sf ABLA} models. Evaporation from {\sf ABLA} model is
    shown below E $\simeq$ 20 MeV. Red and black histograms are the
    {\sf C++} and {\sf FORTRAN} results, respectively. Data points (blue dots) are
    from \cite{data}.}}
...
{\sf ABLA} is an evaporation and fission code developed at GSI, Darmstadt.
\vskip0.5cm
...
{\Large {\sf Comparison of various cascade models in {\sf Geant4}. {\sf INCL/ABLA} (Li\`ege) precision
...
converting the original {\sf FORTRAN} codes {\sf C++} language.
